...
### 5단계. 작품 좋아요 구현
- 작품 좋아요/취소
- 읽기 API와 상세 API의 `liked` 상태 일관성 확인
- 현재 상태: `완료`
...
- 운영 로그와 모니터링 포인트 정리
- 스테이징/운영 환경 변수 분리
- 현재 상태: `기본 테스트/문서 범위 완료`

## 우선순위 이유
- 인증과 사용자 API가 없으면 앱은 로그인과 온보딩을 통과할 수 없다.
- 읽기 API가 먼저 안정되어야 앱 UI와 API 계약을 빨리 고정할 수 있다.
- 작품 좋아요, 코스, 체크인은 읽기 계약이 고정된 뒤 순차적으로 붙여야 재작업이 적다.
- 작품 좋아요를 먼저 분리하면 `liked` 상태의 공통 규칙을 작은 범위에서 먼저 안정화할 수 있다.
- 코스 CRUD와 코스 좋아요는 같은 도메인 경계라 한 단계로 묶고, 체크인은 마지막에 안정화하는 편이 리스크가 낮다.
...
## 한 줄 결론
- SteelArt 서버 구현은 `Node.js 24 + TypeScript + AWS SAM + HTTP API + mysql2 + zod`를 기준으로, 공통 기반과 자동 배포를 먼저 세운 뒤 인증/사용자, 읽기 API, 작품 좋아요, 코스, 체크인 순서로 진행하는 것이 가장 재작업이 적다.
